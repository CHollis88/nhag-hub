...
export const PATCH_NOTES = [
  {
    ...
    items: [
      ...
      "Audio tracks (voice parts, Split Track, Demo) now play with a real, app-styled player instead of Google Drive's own embedded player — streamed through the app's own server for reliability (requires a one-time GOOGLE_DRIVE_API_KEY setup; falls back to Google's player automatically if that's not configured). Tapping a song's link (Sheet Music, Lyrics, Chords) still opens full-screen right in the app instead of leaving to Drive — works the same in Songs and inside a Program's songs. A playing track keeps playing if you switch over to view the Lyrics or Sheet Music at the same time.",
