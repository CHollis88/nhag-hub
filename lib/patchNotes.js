...
export const PATCH_NOTES = [
  {
    ...
    items: [
      ...
      "Audio tracks (voice parts, Split Track, Demo) now play with a real, app-styled player instead of Google Drive's own embedded player — streamed through the app's own server for reliability (requires a one-time GOOGLE_DRIVE_API_KEY setup; falls back to Google's player automatically if that's not configured). Sheet Music, Lyrics, and Chords now render the same way — the browser's own built-in PDF viewer instead of Google's. Tapping a song's link still opens full-screen right in the app instead of leaving to Drive — works the same in Songs and inside a Program's songs. A playing track keeps playing if you switch over to view the Lyrics or Sheet Music at the same time, and forward/back buttons let you skip between a song's voice parts without picking each one from the list. Fixed: once you opened Lyrics or another pdf, there was no way to close it and get back to just the audio — tapping the same pdf button again now toggles it off.",
