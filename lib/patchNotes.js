...
export const PATCH_NOTES = [
  {
    ...
    items: [
      ...
      "Fixed a crash that could happen when closing Lyrics (or another pdf) while a track was also playing.",
      "Fixed pinch-to-zoom on the PDF viewer locking up partway through a pinch — the page now updates live and smoothly through the whole gesture.",
      "Fixed pinch-to-zoom squashing the page narrow once it got zoomed in past screen width — it now scrolls sideways properly instead of getting squeezed.",
      "Hardened the media viewer so a problem loading one file (a track or a pdf) can't crash the whole screen anymore — it now shows a small message in just that spot instead.",
      "Sheet Music, Lyrics, and Chords can now be Word documents (.docx), not just PDFs — they render page-by-page right in the app the same way, with the same page-number box and zoom.",
      ...
      "Fixed a serious crash (sometimes the whole page, not just the viewer) that could happen closing Lyrics while a track was also playing — the PDF viewer no longer gets torn down just from being hidden, only when you actually switch to a different document. Also fixed a version of the same crash on closing the viewer entirely.",
      "Fixed Word documents (.docx) losing all their formatting when viewed inline — headings, spacing, and lists now render the way they actually look in the document.",
